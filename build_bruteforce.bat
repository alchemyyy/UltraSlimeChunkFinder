...
tasklist /FI "IMAGENAME eq slime_bruteforce.exe" 2>nul | find /I "slime_bruteforce.exe" >nul
if %ERRORLEVEL% EQU 0 (
    echo WARNING: slime_bruteforce.exe is currently running.
    set /p "CONFIRM=Kill it and continue? [Y/N]: "
    if /i "!CONFIRM!" NEQ "Y" (
        echo Build cancelled.
        exit /b 1
    )
    taskkill /F /IM slime_bruteforce.exe >nul 2>&1
)
...
echo.
echo Building Slime Brute Force...

cd build

nvcc -O3 -arch=sm_89 -std=c++17 -allow-unsupported-compiler -Xcompiler "/O2" -o ..\slime_bruteforce.exe ..\slime_bruteforce.cu

if %ERRORLEVEL% EQU 0 (
    echo Build successful!
    echo.
    echo Usage: slime_bruteforce.exe ^<baseX^> ^<baseZ^>
) else (
    echo Build failed!
)
