...
tasklist /FI "IMAGENAME eq slime_fft.exe" 2>nul | find /I "slime_fft.exe" >nul
if %ERRORLEVEL% EQU 0 (
    echo WARNING: slime_fft.exe is currently running.
    set /p "CONFIRM=Kill it and continue? [Y/N]: "
    if /i "!CONFIRM!" NEQ "Y" (
        echo Build cancelled.
        exit /b 1
    )
    taskkill /F /IM slime_fft.exe >nul 2>&1
)
...
echo.
echo Building Slime FFT Finder...

cd build
nvcc -O3 -arch=sm_89 -allow-unsupported-compiler -Xcompiler "/openmp /O2" -lcufft -o ..\slime_fft.exe ..\slime_fft.cu

if %ERRORLEVEL% EQU 0 (
    REM Copy cufft DLL - try common naming patterns
    for %%d in ("%CUDA_PATH%\bin\cufft64_*.dll") do (
        copy "%%d" ..\ >nul 2>&1
    )
