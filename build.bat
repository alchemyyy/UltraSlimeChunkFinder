...
tasklist /FI "IMAGENAME eq slime.exe" 2>nul | find /I "slime.exe" >nul
if %ERRORLEVEL% EQU 0 (
    echo WARNING: slime.exe is currently running.
    set /p "CONFIRM=Kill it and continue? [Y/N]: "
    if /i "!CONFIRM!" NEQ "Y" (
        echo Build cancelled.
        exit /b 1
    )
    taskkill /F /IM slime.exe >nul 2>&1
)
...
echo.
echo Building Slime Chunk Finder...

cd build

if exist "..\test_search.cu" (
    echo Building test_search...
    nvcc -O3 -arch=sm_89 -allow-unsupported-compiler -o ..\test_search.exe ..\test_search.cu
)

nvcc -O3 -arch=sm_89 -std=c++17 -allow-unsupported-compiler -Xcompiler "/openmp /O2" -o ..\slime.exe ..\slime.cu
