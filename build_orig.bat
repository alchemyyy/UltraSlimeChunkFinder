...
tasklist /FI "IMAGENAME eq slime_hensel_orig.exe" 2>nul | find /I "slime_hensel_orig.exe" >nul
if %ERRORLEVEL% EQU 0 (
    echo WARNING: slime_hensel_orig.exe is currently running.
    set /p "CONFIRM=Kill it and continue? [Y/N]: "
    if /i "!CONFIRM!" NEQ "Y" (
        echo Build cancelled.
        exit /b 1
    )
    taskkill /F /IM slime_hensel_orig.exe >nul 2>&1
)
...
echo.
echo Building Slime Hensel Original...

cd build

nvcc -O3 -arch=sm_89 -std=c++17 -allow-unsupported-compiler -Xcompiler "/openmp /O2" -o ..\slime_hensel_orig.exe ..\slime_hensel_no_optimization.cu
